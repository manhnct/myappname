...
import hcmut.cse.ten.quickkeyboardviet.VietnameseSpellChecker;
...
public class LatinIME extends InputMethodService implements KeyboardActionListener,
    ...
    private static final int QUICK_PRESS = 150;
    ...
    private CharSequence mHandelEnteredText = null;
 ...
 // position start cursor
    ...
    public void onCreate() {
        ...
        checkVietnameseSubType();        			
        mIsUsingTelexInputMethod = mSettingsValues.mVietnameseInputMethod.equals(
        		VietnameseSpellChecker.INPUT_METHOD_TELEX);
    ...
    private void onStartInputViewInternal(EditorInfo editorInfo, boolean restarting) {
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			case MotionEvent.ACTION_UP:
				inputView.setKeyPreviewPopupEnabled(SettingsValues
						.isKeyPreviewPopupEnabled(mPrefs, mResources), SettingsValues
						.getKeyPreviewPopupDismissDelay(mPrefs, mResources));
				check_long_press = -1;		
				mStartSlide = false;
				break;
...
//        }
    ...
    public void onCodeInput(int primaryCode, int x, int y) {
        ...
        case Keyboard.CODE_DELETE:
        	if(mHandelEnteredText != null){
        		mEnteredText = mHandelEnteredText;
        	}
        ...
        case Keyboard.CODE_LANGUAGE_SWITCH:
            ...
            break;
        case Keyboard.CODE_VIETNAMESE_SWITCH:
            handleVietnameseSwitchKey();
            break;
        default:
        ...
        Log.d(TAG, mKeyboardSwitcher.printKeyboardState());
    }
    ...
    private void handleBackspaceWhileInBatchEdit(final int spaceState, final InputConnection ic) {
        ...
        mHandler.postUpdateShiftState();
        Log.d(TAG, "mEnteredText: " + mEnteredText);
        Log.d(TAG, "mEnteredText_ic: " + ic.getTextBeforeCursor(2, 0));
    ...
    private boolean handleSeparator(final int primaryCode, final int x, final int y,
        ...
        if (mWordComposer.isComposingWord()) {
            ...
            final boolean shouldAutoCorrect = mSettingsValues.mAutoCorrectEnabled
                    ...
                    && (!mIsVietnameseLayout || (mIsVietnameseLayout && !mSettingsValues.mVietnameseOff));            
    ...
    private void handleAfterSeperator(final int primaryCode, final int x, final int y,
        ...
        if (null != ic) ic.beginBatchEdit();
        // TODO: if ic is null, does it make any sense to call this?                
        handleCharacterWhileInBatchEdit(' ', x, y, spaceState, ic);
        if (null != ic) ic.endBatchEdit();
    	return;
    }
    ...
    private int handleVietnameseCharacter_TELEX(int originalTypedChar, int x, int y, int spaceState) {
    	...
    	if (replacedCharIndex != -1) {
            ...
            if (isComposingWord) {
            	...
            	VietnameseSpellChecker.adjustAccent(mWordComposer.getComposingWord(), 0, true);
            	ic.setComposingText(getTextWithUnderline(mWordComposer.getTypedWord()), 1);            	
            	mHandler.postUpdateSuggestions();
            } else {
            	currentWord.setCharAt(replacedCharIndex, (char)replacedChar);
            	VietnameseSpellChecker.adjustAccent(currentWord, 0, true);
    ...
    private int handleVietnameseCharacter_VNI(int originalTypedChar, int x, int y, int spaceState) {
        ...
        StringBuilder currentWord = mTempCurrentWord;        
    	if (!isComposingWord) {
    		//Log.i(TAG, "handle Vietnamese Charater VNI: is Composing Word");
    		currentWord.append(ic.getTextBeforeCursor(10, 0));
    		if (currentWord != null) {
    			for (int i = currentWord.length() - 1; i >= 0; i--) {
    				if (mSettingsValues.isWordSeparator(currentWord.charAt(i))) {
    		...
    		currentWord.append(mWordComposer.getTypedWord());
    		//Log.i(TAG, "handle Vietnamese Charater VNI: currentWord append");
    	...
    	for (int i = 0; i < VNI_DOUBLE_CHARS.length; i++) {
    		if (typedChar == VNI_DOUBLE_CHARS[i]) {
    			typedCharPos = i;
    			break;
    		}
    	}
    	if (typedCharPos != -1) {
			...
			if (!mTypingFreedom) {
				if (currentWord.length() > 0) {
					int charAt = currentWord.charAt(currentWord.length() - 1);
					for (int j = 0; j < VNI_CHARS_TO_FIND[typedCharPos].length; j++) {
						if (charAt == VNI_CHARS_TO_FIND[typedCharPos][j]) {
							replacedCharIndex = currentWord.length() - 1;
							replacedChar = VNI_CHARS_TO_REPLACE[typedCharPos][j];
							if (typedChar == 'w') {
								if (mLastWConverted != 0) {				    				
				    				shouldKeepCurrentChar = false;
				    				replacedChar = mLastWConverted;
			    			...
			    			if (j >= VNI_FALLBACKS[typedCharPos]) {
			    				shouldKeepCurrentChar = true;
			    			}
							break;
						}
				...
				for (int i = currentWord.length() - 1; i >= 0; i--) {
					int charAt = currentWord.charAt(i);
					final int[] charsToFind = VNI_CHARS_TO_FIND[typedCharPos]; 
					for (int j = 0; j < charsToFind.length; j++) {
						if (charAt == charsToFind[j]) {
							replacedCharIndex = i;
							replacedChar = VNI_CHARS_TO_REPLACE[typedCharPos][j];
    		...
    		// process markers: 1 2 3 4 5
    		for (int i = 0; i < VNI_TONE_MARKERS.length; i++) {
        		if (typedChar == VNI_TONE_MARKERS[i]) {
        			typedCharPos = i;
        			break;
        		}
        	}
    ...
    private void checkVietnameseSubType() {
		final InputMethodSubtype subType = mSubtypeSwitcher.getCurrentSubtype();		
		mIsVietnameseLayout = "vietnamese".equals(SubtypeLocale.getKeyboardLayoutSetName(subType));        
        mIsVietnameseSubType = "vi".equals(subType.getLocale())
        		&& ((mIsVietnameseLayout && !mSettingsValues.mVietnameseOff) || !mIsVietnameseLayout);
	}
    
    private boolean shouldHandleVietnameseCharacter(int primaryCode) {
    	if (mIsUsingTelexInputMethod )
    		return mIsVietnameseSubType && isVietnameseToneMarker_Telex(primaryCode);
    	if (mIsUsingVNIInputMethod )
    		return mIsVietnameseSubType && isVietnameseToneMarker_VNI(primaryCode);
    	return false;	
    }
    
    private boolean isVietnameseToneMarker_Telex(int typedChar) {
    	return 
    		typedChar == 'a' || // aa -> â 
    		typedChar == 'e' || // ee -> ê 
    		typedChar == 'o' || // oo -> ô
    ...
    private boolean isVietnameseToneMarker_VNI(int typedChar) {
    	return 
    		typedChar == '6' || // aa -> â, oo -> ô, ee -> ê
    		typedChar == '7' || // uw -> ư, ow -> ơ
    		typedChar == '8' || // aw -> ă
    		typedChar == '9' || // dd -> đ;
    		typedChar == '0' || // á -> a
    		typedChar == '1' || // a1 -> á
    ...
    private void handleVietnameseSwitchKey() {
    	mSettingsValues.mVietnameseOff = !mSettingsValues.mVietnameseOff;
    	
    	// save preference
    	Editor editor = mPrefs.edit();
    	editor.putBoolean("vietnamese_off", mSettingsValues.mVietnameseOff);
    	editor.commit();
    	
    	// need to update keyboard
    	if (mKeyboardSwitcher.getKeyboardView() != null) {    		
            mKeyboardSwitcher.loadKeyboard(getCurrentInputEditorInfo(), mSettingsValues);
        }
    ...
    private final static int[] VNI_DOUBLE_CHARS = { '6', '7', '8', '9'};
	...
	private static final int[][] VNI_CHARS_TO_FIND = {
    	{ 'a', 'á', 'à', 'ả', 'ã', 'ạ',		/* 6 */
    	  'A', 'Á', 'À', 'Ả', 'Ã', 'Ạ',
    	  'â', 'ấ', 'ầ', 'ẩ', 'ẫ', 'ậ', 
    	  'Â', 'Ấ', 'Ầ', 'Ẩ', 'Ẫ', 'Ậ',
	...
	private static final int[][] VNI_CHARS_TO_REPLACE = {
    	{ 'â', 'ấ', 'ầ', 'ẩ', 'ẫ', 'ậ',	 /* 6 */
    	  'Â', 'Ấ', 'Ầ', 'Ẩ', 'Ẫ', 'Ậ',
    	  'a', 'á', 'à', 'ả', 'ã', 'ạ', 
    	  'A', 'Á', 'À', 'Ả', 'Ã', 'Ạ',
    ...
    private static final int[] VNI_FALLBACKS = {
    	96,
    	60,
    	36,
    	2
    ...
    private static final int[] VNI_TONE_MARKERS = { '0', '1', '2', '3', '4', '5' };
